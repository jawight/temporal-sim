...
## Key Behaviors
- **Synchronized Replay**: Logic tasks trigger a sequential highlight of all prior events in the Event History to show how the system recovers state. When a worker is killed but the workflow execution is not completed, drop the existing task from the queue and start at the beginning. Then instead of doing each step, create an animation that highlights different elements for 1 second. Start from the beginning of the event log. Then Highlight the first task in the workflow definition by making it's border glow yellow for one second. Highlight the first corresponding completed task in the event history by making a glowing yellow border around it's logs for 1 second. Then highlight the next task in the workflow definition for 1 second. Then highlight the corresponding event history logs for 1 second. Repeat until there are no logs associated with the workflow task and add that one to the task queue for a worker to pick up.
